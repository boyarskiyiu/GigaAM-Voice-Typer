...
# -*- coding: utf-8 -*-
"""
GigaAM Complete — Версия 1.5.0 (15.04.2026)
(c) Боярский Игорь Юрьевич, 2026

- Исправлено сравнение версий (теперь версия программы синхронизирована с тегами GitHub)
"""
...
# ----------------------------------------------------------------------
lock_file = os.path.join(tempfile.gettempdir(), "gigaam_324.lock")
...
class GigaAMApp:
    ...
    def create_widgets(self):
        ...
        header = tk.Frame(header_frame, bg="#2c3e50", height=200, relief=tk.RAISED, borderwidth=3)
        ...
        tk.Label(right, text="✉️ boyarskiyiu@yandex.ru", font=("Segoe UI", 11),
                 bg="#2c3e50", fg="#ecf0f1").place(relx=0.98, y=42, anchor="ne")
        tk.Label(right, text="© 2026 Боярский И.Ю.\nВсе права защищены.",
                 font=("Segoe UI", 10), bg="#2c3e50", fg="#bdc3c7", justify=tk.RIGHT).place(relx=0.98, y=80, anchor="ne")
        ...
        self.volume_indicator = ttk.Progressbar(vol_frame, mode='determinate', length=90, maximum=100)
        ...
        phrase_frame.pack(fill=tk.X, padx=8, pady=4)
        ...
        self.btn_exit.pack(side=tk.RIGHT, padx=3)
    ...
    def compare_versions(self, v1, v2):
        """Сравнивает версии формата X.Y.Z. Возвращает 1 если v1 > v2, -1 если v1 < v2, 0 если равны."""
        def normalize(v):
    ...
    def check_updates(self):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                self.log(f"   Попытка {attempt}...")
                response = requests.get(url, timeout=10)
                response.raise_for_status()
                latest_release = response.json()
                latest_version = latest_release["tag_name"].replace("v", "")
                self.log(f"   Версия на GitHub: {latest_version}")
                self.log(f"   Текущая версия: {CURRENT_VERSION}")

                cmp = self.compare_versions(latest_version, CURRENT_VERSION)
                if cmp > 0:
                    self.log(f"   ✅ Доступна новая версия!")
                    ...
                    self.log(f"   ℹ️ Ваша версия новее, чем на GitHub (возможно, бета).")
                    messagebox.showinfo("Обновлений нет", "У вас установлена версия, которая новее официального релиза.")
                else:
                    self.log("   ℹ️ У вас последняя версия.")
                    messagebox.showinfo("Обновлений нет", "У вас установлена последняя версия.")
                return
            except requests.exceptions.ConnectionError as e:
                self.log(f"   ⚠️ Сетевая ошибка (попытка {attempt}/{max_retries}): {e}")
    ...
    def show_about(self):
        about_text = (
            ...
            f"Версия {CURRENT_VERSION} (15.04.2026)\n\n"
            "Разработчик: Боярский Игорь Юрьевич\n"
            "© 2026 Все права защищены.\n\n"
            "Функции:\n"
            "• Распознавание речи моделью GigaAM-v3\n"
            "• Яндекс.Спеллер (опечатки, повторы)\n"
            "• Автоустановка зависимостей и модели\n"
            "• Автоматический выбор микрофона\n"
            "• Защита от повторного запуска\n"
            "• Длинные фразы без потерь\n"
    ...
    def fix_last_phrase(self):
        ...
        wrong_entry = tk.Entry(dialog, width=70, font=("Segoe UI", 11))
        ...
        correct_entry = tk.Entry(dialog, width=70, font=("Segoe UI", 11))
    ...
    def init_background(self):
        self.log("="*60)
        self.log(f"GigaAM Complete v{CURRENT_VERSION} — сравнение версий исправлено")
        self.log(f"Задействовано ядер CPU: {cpu_cores}")
        self.log("(c) Боярский Игорь Юрьевич, 2026")
        if self.speller:
            self.log("✅ Яндекс.Спеллер активирован")
        else:
            self.log("⚠️ Яндекс.Спеллер недоступен")
        self.log("="*60)
