...
# -*- coding: utf-8 -*-
"""
GigaAM Complete — Версия 1.8.6 (15.04.2026)
(c) Боярский Игорь Юрьевич, 2026

- Полностью пересчитана геометрия: окно 730x740, ничего не налезает
- Кнопки шириной 13, шрифт 9pt – текст влезает идеально
- Сохранены цвета кнопок, автообновление, Яндекс.Спеллер
"""
...
# ----------------------------------------------------------------------
lock_file = os.path.join(tempfile.gettempdir(), "gigaam_186.lock")
...
class GigaAMApp:
    ...
    def create_widgets(self):
        ...
        header = tk.Frame(header_frame, bg="#2c3e50", height=210, relief=tk.RAISED, borderwidth=3)
        ...
        tk.Label(right, text="© 2026 Боярский И.Ю.\nВсе права защищены.",
                 font=("Segoe UI", 10, "bold"), bg="#2c3e50", fg="#f1c40f", justify=tk.RIGHT).place(relx=0.98, y=72, anchor="ne")
        ...
        self.volume_indicator = ttk.Progressbar(vol_frame, mode='determinate', length=65, maximum=100)
        ...
        # Кнопки – ширина 13, шрифт 9pt bold
    ...
    def fix_last_phrase(self):
        ...
        tk.Label(dialog, text="Неправильно:", bg="#f0f0f0", font=("Segoe UI", 10)).pack(pady=(10,0))
        wrong_entry = tk.Entry(dialog, width=50, font=("Segoe UI", 10))
        wrong_entry.insert(0, self.last_orig)
        wrong_entry.config(state='readonly')
        wrong_entry.pack(pady=5)
        tk.Label(dialog, text="Правильный текст:", bg="#f0f0f0", font=("Segoe UI", 10)).pack()
        correct_entry = tk.Entry(dialog, width=50, font=("Segoe UI", 10))
        ...
        btn_save = tk.Button(dialog, text="Сохранить", command=lambda: self._save_fix(dialog, correct_entry.get().strip()),
                             bg="#4caf50", fg="white", width=13, font=("Segoe UI", 9, "bold"))
